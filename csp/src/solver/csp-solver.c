...
bool csp_problem_backtrack(const CSPProblem *csp, size_t *values,
													 ...
													 Domain **domains, DomainChange *change_stack,
													 size_t *stack_top) {
	assert(csp_initialised());
	backtrack_counter++;
	...
	for (size_t i = 0; i < domains[index]->amount; i++) {
		...
		bool result;
		if (solve_type & FC) {
			result = csp_problem_forward_check(csp, values, data, index, fv, checklist,
																		domains, change_stack, stack_top) &&
					csp_problem_backtrack(csp, values, data, fv, solve_type, checklist,
																domains, change_stack, stack_top);
		} else {
			result = csp_problem_is_consistent(csp, values, data, index, fv, checklist) &&
					csp_problem_backtrack(csp, values, data, fv, solve_type, checklist,
																domains, NULL, NULL);
		}
...
bool csp_problem_solve(const CSPProblem *csp, size_t *values, const void *data,
	...
	if (solve_type & FC) {
		...
		result = csp_problem_backtrack(csp, values, data, fv, solve_type, checklist,
																	 domains, change_stack, &stack_top);
