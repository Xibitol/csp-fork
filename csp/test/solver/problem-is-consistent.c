...
int test_solver_problem_is_consistent(void){
	...
	csp_init();
	{
		...
		csp_problem_set_constraint(problem, 0,
			csp_constraint_create(2, (CSPChecker *)different)
		);
		csp_constraint_set_variable(
			csp_problem_get_constraint(problem, 0), 0, 0
		);
		csp_constraint_set_variable(
			csp_problem_get_constraint(problem, 0), 1, 1
		);
